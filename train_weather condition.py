...
# Xếp loại thời tiết
list_weather = ['Light drizzle', 'Light rain', 'Light rain shower', 'Patchy light drizzle', 'Patchy light rain',
             ...
             'Torrential rain shower', 'Clear', 'Cloudy', 'Mist', 'Sunny', 'Partly cloudy', 'Thundery outbreaks possible']

y = []
for w in data.weather:
    if w in list_weather:
        y += [w]
    else:
        y += ['unknown']
...
# Thêm 'weather' vào danh sách cột số
nume_cols = ['temperature', 'feelslike', 'wind', 'gust', 'cloud', 'humidity', 'pressure']

# Thêm 'weather' vào danh sách cột phân loại
...
# Cập nhật Pipeline tiền xử lý
cols_trans = ColumnTransformer(transformers=[
    ('nume', nume_trans, nume_cols),
    ('cate', cate_trans, cate_cols)
])

# Cập nhật preprocess_pipeline
preprocess_pipeline = Pipeline(steps=[
    ('colstd', ColStd()),
    ('coltrans', cols_trans),
    ('std', StandardScaler(with_mean=False))  # Đặt with_mean=False
])
...
joblib.dump(mlpclf_model, "mlpclf_model1.pkl")

# Đánh giá mô hình MLPClassifier trên tập kiểm tra
y_pred_mlp = mlpclf_model.predict(x_test)
print("mean_error",mean_squared_error(y_test,y_pred_mlp))
...
joblib.dump(svmclf_model, "svmclf_model1.pkl")
...
joblib.dump(best_gb_model, "gb_model1.pkl")
